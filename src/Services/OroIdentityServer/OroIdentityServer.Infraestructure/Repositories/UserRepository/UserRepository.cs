    ...
    IPasswordHasher passwordHasher) : IUserRepository
{
    ...
    public async Task<bool> ChangePasswordAsync(string email, string currentPassword, string newPassword, string confirmedPassword, CancellationToken cancellationToken)
    {
        ...
        if (user.SecurityUserId == null)
        {
            logger.LogWarning("User {Email} does not have a SecurityUser assigned", email);
            return false;
        }

        var securityUser = await securityUserRepository.GetSecurityUserAsync(user.SecurityUserId.Value, cancellationToken);

        var isCurrentPasswordValid = await passwordHasher.VerifyPassword(currentPassword, securityUser.PasswordHash ?? string.Empty);
        ...
        var newHashed = await passwordHasher.HashPassword(newPassword);
        securityUser.ChangePassword(newHashed);

        await securityUserRepository.UpdateSecurityUserAsync(securityUser, cancellationToken);
