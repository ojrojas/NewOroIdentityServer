    ...
    IRepository<Role> repository) : IRolesRepository
{
    ...
    public async Task<RoleClaim?> GetRoleClaimByIdAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        var roleClaim = await repository.CurrentContext
    ...
    public async Task DeleteRoleClaimAsync(Guid id, CancellationToken cancellationToken)
    {
        ...
        var role = await repository.CurrentContext.FirstOrDefaultAsync(r => r.Claims.Any(c => c.Id == id), cancellationToken);
        if (role != null)
        {
    ...
    public async Task UpdateRoleClaimAsync(Guid claimId, RoleClaimType newClaimType, RoleClaimValue newClaimValue, CancellationToken cancellationToken)
    {
        ...
        var role = await repository.CurrentContext.FirstOrDefaultAsync(r => r.Claims.Any(c => c.Id == claimId), cancellationToken);
        if (role != null)
        {
